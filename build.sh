...
function usage() {
    cat << EOF
Builds and installs the Montecristo and DSE stats converter binaries.
...
while getopts "cd:h" opt_flag; do
    case $opt_flag in
        c) clean_build="true" ;;
...
if [ -f "${SCRIPT_DIR}/dse-stats-converter/build.gradle" ]; then
    ...
    pushd "${SCRIPT_DIR}/dse-stats-converter" || exit 1
    if ./gradlew installDist -x test -PinstallPath="${DSE_STATS_INSTALL_DIR}"; then
        popd || exit 1
